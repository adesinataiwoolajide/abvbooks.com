...
try{
    if(isset($_POST['add-genre'])){
        ...
        echo $genre_name = $all_purpose->sanitizeInput($_POST['genre_name']);
        echo$type_id = $all_purpose->sanitizeInput($_POST['type_id']);
        $totalGenre = count($genre->checkIfAlreadyAdded($genre_name));

        if($totalGenre>0){
            $_SESSION['error']= "You Have Added $genre_name  Before";
            $all_purpose->redirect($return);
        }else{
            if($genre->createGenre($genre_name, $type_id)){
                $action ="Added $genre_name  to the Author List";
